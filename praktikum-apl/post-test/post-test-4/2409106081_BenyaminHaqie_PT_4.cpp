...
void tambahAkun() {
    if (jumlahAkun < MAX_AKUN) {
        ...
        cin.ignore();
        getline(cin, daftarAkun[jumlahAkun].id);
...
void updateAkun() {
    ...
    if (index > 0 && index <= jumlahAkun) {
        ...
        cin.ignore();
        getline(cin, daftarAkun[index - 1].id);
...
void sewaAkun() {
    ...
    cin >> index;
    if (index > 0 && index <= jumlahAkun) {
        cout << "Anda telah menyewa akun " << daftarAkun[index - 1].id << " dengan harga " << daftarAkun[index - 1].harga << "\n";
    } else {
        cout << "Nomor akun tidak valid!\n";
    }
}
